...
import (
	"fmt"
	commentproto "user-service/genproto/comment_service"
...
type ServiceClients interface {
	...
	CommentService() commentproto.CommentServiceClient
	Close()
}
...
func New(config *config.Config) (ServiceClients, error) {
	...
	connCommentService, err := grpc.Dial(
		fmt.Sprintf("%s%s", config.CommentService.Host, config.CommentService.Port),
		grpc.WithInsecure(),
		// grpc.WithUnaryInterceptor(otelgrpc.UnaryClientInterceptor()),
		// grpc.WithStreamInterceptor(otelgrpc.StreamClientInterceptor()),
	)
	...
	return &serviceClients{
		...
		commentService: commentproto.NewCommentServiceClient(connCommentService),
		services:       []*grpc.ClientConn{},
	}, nil
}
...
func (s *serviceClients) CommentService() commentproto.CommentServiceClient {
